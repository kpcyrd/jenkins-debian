...
write_page "        <em>Reproducible Coreboot</em> is an effort to apply this to coreboot. Thus each coreboot.rom is build twice, with a few varitations added and then those two ROMs are compared using <a href=\"https://tracker.debian.org/debbindiff\">debbindiff</a>. Please note that more variations are to be expected in the wild.</p>"
write_page "       <p>There is a monthly ru <a href=\"https://jenkins.debian.net/view/reproducible/job/reproducible_coreboot/\">jenkins job</a> to test the <code>master</code> branch of <a href=\"https://review.coreboot.org/p/coreboot.git\">coreboot.git</a>, but as this is brand new, this job is currently triggered more often. This job runs <a href=\"http://anonscm.debian.org/cgit/qa/jenkins.debian.net.git/tree/bin/reproducible_coreboot.sh\">reproducible_coreboot.sh</a> which is solely responsible for creating this page. Please join <code>#debian-reproducible</code> (on irc.oftc.net) to request job runs whenever sensible. Patches are very much welcome!</p>"
write_page "       <p>These tests were last run on $DATE for version ${COREBOOT_VERSION}.</p><p><pre>"
