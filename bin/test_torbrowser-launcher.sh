...
upgrade_to_package_build_from_git() {
	...
	# build package
	schroot --run-session -c $SESSION --directory $TMPDIR/git -- gbp buildpackage --git-ignore-branch --git-upstream-branch=origin/master --git-upstream-tree=branch -uc -us
	# install it
